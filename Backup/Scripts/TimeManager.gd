# TimeManager.gd - Core turn management system
...
func process_turn():
	...
	while true:
		...
		if not next_actor:
			...
			break
		print("Next Actor: ", next_actor.name)
...
func process_actor_turn(actor: Actor) -> void:
	...
	if action:
		...
		actor.time_accumulated += BASE_TIME_UNIT
...
func execute_action(actor: Actor, action: Action) -> int:
	var time_cost = action.get_time_cost()
...
func get_turns_until_actor_acts(target_actor: Actor) -> int:
	...
	while temp_actors[target_actor] > 0:
		...
		# Add action time cost
		temp_actors[next_actor] += BASE_TIME_UNIT
		turns += 1
